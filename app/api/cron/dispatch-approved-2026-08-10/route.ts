...
const SCHEDULE_DATE = "2026-08-10";
...
export async function GET(request: NextRequest) {
  ...
  if (now.date !== SCHEDULE_DATE || now.hour !== "08") return NextResponse.json({ started: false, reason: "Fora da data e hora autorizadas" });
  const [campaign, totalItems, uniquePhones] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ count: bigint }>>`select count(distinct c.phone)::bigint count from bot_campaign_items bci join case_records cr on cr.id=bci.case_id join service_orders so on so.id=cr.service_order_id join customers c on c.id=so.customer_id where bci.campaign_id=${CAMPAIGN_ID}::uuid`,
  ]);
  const uniquePhoneCount = Number(uniquePhones[0]?.count ?? 0);
  if (!campaign || campaign.name !== CAMPAIGN_NAME || campaign.status !== "RASCUNHO" || campaign.maxSendPerRun !== HARD_LIMIT || campaign.maxAttempts !== 1 || totalItems !== HARD_LIMIT || uniquePhoneCount !== HARD_LIMIT) return NextResponse.json({ started: false, reason: "Lote bloqueado pela validação", totalItems, uniquePhoneCount }, { status: 409 });

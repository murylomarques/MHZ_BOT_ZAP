...
const { autoAssignToRoute } = require('./motoboy-routing');
...
// Endereço já cadastrado (da importação de planilha) pra esse telefone —
// usado pra perguntar "seu endereço é esse?" em vez de pedir pra digitar de
// novo, reduzindo desistência no fluxo de retirada.
async function getKnownAddress(waId) {
  try {
    const pool = getPool();
    const { rows } = await pool.query(
      `select ca.full_address
       from customers c
       join customer_addresses ca on ca.customer_id = c.id
       where c.phone = $1
       order by (ca.kind = 'original') desc, ca.created_at desc
       limit 1`,
      [waId]
    );
    return rows[0]?.full_address || null;
  } catch (err) {
    console.error('Erro ao buscar endereço já cadastrado:', err.message);
    return null;
  }
}
...
async function syncPickupScheduled({ waId, pickupDate, period, address, contactPhone, observacao }) {
  try {
    ...
    if (!LATE_PICKUP_STATUSES.includes(status)) {
      ...
      // Agendamento confirmado de verdade pelo cliente — roteiriza automaticamente
      // pro motoboy que atende essa cidade, sem precisar copiar/colar num grupo.
      await autoAssignToRoute(caseId);
    }
  } catch (err) {
...
// Cliente já escolheu dia + período mas ainda não confirmou endereço/
// telefone/observação (pode ter parado de responder aí) — mesmo assim já
// conta como "agendado" pro time de operação, usando o endereço e telefone
// do cadastro como palpite. confirmed_by_client = false marca isso como
// PARCIAL nas telas; se o cliente completar o fluxo depois, syncPickupScheduled
// atualiza esse mesmo appointment com os dados reais e confirmed_by_client = true.
async function syncPartialSchedule({ waId, pickupDate, period }) {
  try {
    const pool = getPool();
    const openCase = await findOpenCaseByPhone(pool, waId);
    if (!openCase) return;
    ...
    const knownAddress = addressRows[0]?.full_address;
    if (!knownAddress) return; // sem endereço de cadastro, não dá pra montar o parcial

    const window = PERIOD_WINDOWS[period] || { start: '08:00', end: '18:00' };
    const { rows: existing } = await pool.query(`select id, confirmed_by_client from appointments where case_id = $1`, [caseId]);
    // Se já existe um agendamento confirmado de verdade, não sobrescreve com o parcial.
    if (existing.length && existing[0].confirmed_by_client) return;

    await pool.query(
      `insert into appointments (id, case_id, date, window_start, window_end, address, observation, confirmed_by_client, created_at, updated_at)
       values (gen_random_uuid(), $1, $2, $3, $4, $5, null, false, now(), now())
       on conflict (case_id) do update set
         date = excluded.date,
         window_start = excluded.window_start,
         window_end = excluded.window_end,
         ...
         updated_at = now()`,
      [caseId, pickupDate, window.start, window.end, knownAddress]
    );
    ...
    if (!LATE_PICKUP_STATUSES.includes(status)) {
      ...
      await pool.query(
        `insert into case_status_history (id, case_id, from_status, to_status, origin, reason, created_at)
         values (gen_random_uuid(), $1, $2, 'AGENDADO', 'BOT', 'Agendamento parcial (cliente escolheu dia/período, aguardando confirmação do endereço)', now())`,
        [caseId, status]
      );
    }
  } catch (err) {
    console.error('[new-schema-sync] erro ao sincronizar agendamento parcial:', err.message);
  }
}
...
// Registra um passo da conversa do bot (fluxo de retirada ou de retenção)
// pra alimentar o gráfico de funil/abandono no dashboard — ver
// bot_funnel_events e lib/server/status/bot-funnel.ts.
async function logFunnelStep(waId, step) {
  try {
    const pool = getPool();
    await pool.query(
      `insert into bot_funnel_events (id, wa_id, step, created_at) values (gen_random_uuid(), $1, $2, now())`,
      [waId, step]
    );
  } catch (err) {
    console.error('Erro ao registrar evento de funil do bot:', err.message);
  }
}

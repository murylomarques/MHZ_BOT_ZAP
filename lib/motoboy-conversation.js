...
// parada por parada, marcar retirado ou cliente ausente (com motivo/
// observação) — sem depender do time de operação. Mesmo padrão CommonJS +
// pg cru de lib/new-schema-sync.js / lib/motoboy-routing.js.
const { getPool } = require('./db');
const { getConversationState, setConversationState, clearConversationState } = require('./db');
...
async function sendCurrentStopOrFinish(waId, pool, courierId) {
  ...
  // Motoboy está a caminho dessa parada agora.
  if (stop.case_status === 'ATRIBUIDO_MOTOBOY') {
    await transitionStatus(pool, stop.case_id, 'EM_DESLOCAMENTO', 'Motoboy a caminho (WhatsApp)');
  }
  await pool.query(`update route_stops set status = 'EM_ANDAMENTO' where id = $1 and status = 'PENDENTE'`, [stop.stop_id]);

  await sendButtons(waId, formatStopMessage(stop), [
...
async function resolveStopAndAdvance(waId, pool, stopCtx, outcome, reason, note) {
  ...
  const { rows: pickupRows } = await pool.query(
    `insert into pickups (id, case_id, performed_at, result, observation, created_at)
     values (gen_random_uuid(), $1, now(), $2, $3, now())
     on conflict (case_id) do update set performed_at = now(), result = excluded.result, observation = excluded.observation
     returning id`,
    [caseId, outcome, note || null]
  );
  const pickupId = pickupRows[0].id;

  if (outcome === 'retirado') {
    await pool.query(
      `insert into system_closures (id, pickup_id, created_at, updated_at)
...
async function handleMotoboyConversation({ waId, msg, courier }) {
  ...
  if (!state) {
    await sendButtons(waId, `Oi, ${courier.name}! 👋`, [{ id: 'mb_ver_rota', title: 'Ver rota de hoje' }]);
  ...
  if (state === 'MB_MENU') {
    ...
    await sendButtons(waId, 'Como posso ajudar?', [{ id: 'mb_ver_rota', title: 'Ver rota de hoje' }]);
    return;
  }

  if (state === 'MB_VIEWING_STOP') {
    if (input.kind !== 'interactive') {
      await sendButtons(waId, 'Escolhe uma opção pra essa parada:', [
        { id: 'mb_retirado', title: 'Retirado' },
        { id: 'mb_ausente', title: 'Cliente ausente' },
        { id: 'mb_outro', title: 'Outro problema' },
      ]);
    ...
    if (input.id === 'mb_retirado') {
      await resolveStopAndAdvance(waId, pool, data, 'retirado');
...
module.exports = { handleMotoboyConversation };

...
const { notifyTodayAppointmentForCustomer } = require('./dispatch-control');
...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  if (state === 'AWAITING_PERIOD') {
    ...
    await goTo(waId, 'AWAITING_OBSERVATION', { ...data, period, contactPhone: waId });
    return;
  }
  ...
  if (state === 'AWAITING_CONFIRM') {
    ...
    if (input.id === 'confirm_yes') {
      ...
      await notifyTodayAppointmentForCustomer(waId);
      await sendText(
        waId,

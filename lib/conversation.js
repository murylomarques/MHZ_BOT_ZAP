...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  if (state === 'AWAITING_HOME_CONFIRM') {
    ...
    await sendText(waId, isHome ? 'Obrigado! Já avisei o motoboy. 🛵' : 'Combinado, já avisei o motoboy. 🛵');
    if (data.motoboyPhone) {
      await sendText(
        data.motoboyPhone,
        isHome ? '🏠 Cliente confirmou: está em casa.' : '⚠️ Cliente disse que NÃO está em casa — confirme antes de fazer a viagem.'
      );
    }
    await clearConversationState(waId);
    return;

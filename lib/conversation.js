...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  if (input.kind === 'button') {
    const buttonText = normalize(input.payload || input.text);
    if (buttonText === 'comecar' || buttonText === 'agendar retirada') {
...
module.exports = { handleConversation, startPickupFlow, BRAND };

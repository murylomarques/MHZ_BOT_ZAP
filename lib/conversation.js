...
async function askRetentionOffer(waId) {
  await sendButtons(
    waId,
    'Antes de seguir com a retirada, quero te fazer uma proposta 🙂\n\n' +
      'Conseguimos condições especiais para você continuar com a internet — inclusive dá pra fazer um novo contrato ' +
      'no nome de outra pessoa (um familiar, por exemplo), caso isso ajude.\n\n' +
      'Quer que eu veja uma condição especial pra você continuar?',
    [
...
const RETENTION_STEPS = [
  ...
  { state: 'AWAITING_RET_PLANO', field: 'plano', prompt: 'Qual plano você tem interesse (ex: velocidade ou o que já conhece da MHZ)?' },
...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  if (state === 'AWAITING_RETENTION_OFFER') {
    ...
    if (input.id === 'retention_yes') {
      ...
      await setConversationState(waId, RETENTION_STEPS[0].state, {});
      return;
    }

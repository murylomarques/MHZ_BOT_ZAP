...
const {
  ...
  logFunnelStep,
  getKnownAddress,
} = require('./new-schema-sync');
const { handleCustomerNotHome } = require('./motoboy-conversation');
...
function isCepOnly(text) {
  return /^\d{5}-?\d{3}$/.test((text || '').trim());
}

// Cliente manda só o CEP em vez do endereço completo — busca rua/bairro/
// cidade via ViaCEP e só pede o número/complemento, em vez de rejeitar
// e pedir pra digitar tudo de novo.
async function lookupCep(text) {
  try {
    const cep = (text || '').replace(/\D/g, '');
    const res = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
    if (!res.ok) return null;
    const json = await res.json();
    if (json.erro || !json.logradouro) return null;
    const parts = [json.logradouro, json.bairro, json.localidade && json.uf ? `${json.localidade} - ${json.uf}` : null].filter(Boolean);
    return `${parts.join(', ')}, CEP ${json.cep}`;
  } catch (err) {
...
async function showPickupOpening(waId) {
  await sendButtons(
    waId,
    'Vamos agendar sua retirada\n\nO processo possui apenas 3 etapas e leva menos de 1 minuto:\n\n1. Confirmar o endereço\n2. Escolher a data e o período\n3. Revisar e confirmar',
    [{ id: 'pickup_start', title: 'Começar' }]
  );
}

async function startPickupFlow(waId) {
  const knownAddress = await getKnownAddress(waId);
  if (knownAddress) {
    await askAddressConfirm(waId, knownAddress);
    await goTo(waId, 'AWAITING_ADDRESS_CONFIRM', { knownAddress });
    return;
  }
  await askAddress(waId);
  await goTo(waId, 'AWAITING_ADDRESS', {});
}

async function askDate(waId) {
  const today = nowSaoPaulo().dateIso;
  const options = [];
  if (availablePeriodsForDate(today).length) options.push({ id: `date_${today}`, title: 'Hoje — mais rápido' });
  options.push({ id: 'date_other', title: 'Escolher outra data' });
  await sendButtons(
    waId,
    'Etapa 2 de 3: data\n\nQuando podemos retirar?\n\nEscolha quando haverá alguém no endereço para entregar os equipamentos.',
    options
  );
}

async function askOtherDate(waId) {
  const today = nowSaoPaulo().dateIso;
  const dates = nextDates(5).filter((date) => date.iso !== today).slice(0, 3);
  await sendList(
    waId,
    'Escolha outra data\n\nSelecione a data mais próxima em que haverá alguém no endereço.',
    'Ver datas',
    [{ title: 'Próximas datas', rows: dates.map((d) => ({ id: `date_${d.iso}`, title: d.label })) }]
  );
}
...
async function askPeriod(waId, periods) {
  await sendButtons(
    waId,
    'Em qual período haverá alguém no endereço?',
    periods.map((p) => ({ id: p.id, title: p.title }))
  );
}

async function askAddressConfirm(waId, knownAddress) {
  await sendButtons(waId, `Etapa 1 de 3: endereço\n\nConfirme o endereço\n\nLocalizamos o seguinte endereço para retirada:\n\n📍 ${knownAddress}\n\nA retirada será realizada neste endereço?`, [
    { id: 'address_yes', title: 'Sim, usar endereço' },
    { id: 'address_no', title: 'Alterar endereço' },
  ]);
}
...
async function askAddress(waId) {
  await sendText(
    waId,
    'Informe o endereço da retirada\n\nEnvie o CEP ou o endereço completo (rua, número, complemento, bairro e cidade).'
  );
}
...
async function askConfirmation(waId, data) {
  const summary =
    ...
    `Responsável: ${data.responsibleName || 'Eu mesmo'}\n` +
    `Equipamentos: modem/roteador, fonte e cabos/acessórios informados\n\n` +
    `Ao confirmar, sua solicitação será enviada para programação da equipe de retirada.`;

  await sendButtons(waId, summary, [
    { id: 'confirm_yes', title: 'Confirmar retirada' },
    { id: 'confirm_no', title: 'Alterar informações' },
  ]);
}

async function askResponsible(waId) {
  await sendButtons(waId, 'Quem entregará os equipamentos?\n\nEscolha quem estará no endereço durante o período informado.', [
    { id: 'responsible_self', title: 'Eu mesmo' },
    { id: 'responsible_other', title: 'Outra pessoa' },
  ]);
}

async function askEquipmentPreparation(waId) {
  await sendButtons(
    waId,
    'Deixe os itens separados\n\nPara agilizar a retirada, deixe separados:\n\n• Modem ou roteador\n• Fonte de energia\n• Cabos e acessórios informados\n\nVocê consegue deixar esses itens separados até a retirada?',
    [
      { id: 'equipment_ready', title: 'Sim, vou separar' },
      { id: 'equipment_help', title: 'Preciso de ajuda' },
    ]
  );
}

async function askEquipmentHelp(waId) {
  await sendList(waId, 'Como podemos ajudar?', 'Escolher problema', [{
    title: 'Ajuda com os equipamentos',
    rows: [
      { id: 'help_not_found', title: 'Não encontrei o item' },
      { id: 'help_missing_part', title: 'Falta fonte ou cabo' },
      { id: 'help_returned', title: 'Já foi devolvido' },
      { id: 'help_unknown', title: 'Não reconheço' },
      { id: 'help_other', title: 'Outro problema' },
    ],
  }]);
}
...
// Botões de quick-reply dos templates: a Meta manda o texto do
...
// Avança o cadastro de retenção a partir do estado (campo) atual — usado
// tanto no fluxo normal (digitou o campo) quanto no atalho de CEP (digitou
// só o CEP, número/complemento vem numa etapa própria antes de chamar isso).
async function advanceRetentionFlow(waId, contactId, fromState, updated) {
  const idx = retentionStepIndex(fromState);
  const nextIdx = idx + 1;
  ...
  await syncRetentionLead({
    waId,
    motivoLabel: RETENTION_REASONS.find((r) => r.id === updated.motivo)?.title,
    ...updated,
  });
...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  // O template msg_inicio_foco_retiradas entra diretamente no agendamento.
  if (input.kind === 'button') {
    if (normalize(input.payload) === 'comecar' || normalize(input.text) === 'comecar') {
  ...
  if (!state) {
    ...
    await goTo(waId, 'AWAITING_PICKUP_START', {});
    return;
  }

  if (state === 'AWAITING_PICKUP_START') {
    if (input.kind !== 'interactive' || input.id !== 'pickup_start') {
      await showPickupOpening(waId);
      return;
    }
    await startPickupFlow(waId);
  ...
  if (state === 'AWAITING_RET_PLANO') {
    ...
    await advanceRetentionFlow(waId, contactId, 'AWAITING_RET_PLANO', { ...data, plano: plan.label });
    return;
  }

  if (state === 'AWAITING_RET_ENDERECO_NUMERO') {
    if (input.kind !== 'text' || !input.value.trim()) {
      await sendText(waId, 'Só preciso do número da casa/apartamento e complemento (se houver) pra ficar completo.');
      return;
    }
    const endereco = `${data.cepBaseAddress} - nº ${input.value.trim()}`;
    await advanceRetentionFlow(waId, contactId, 'AWAITING_RET_ENDERECO', { ...data, endereco });
    return;
  }
  ...
  if (state && state.startsWith('AWAITING_RET_')) {
    ...
    // Endereço: se mandou só o CEP, busca rua/bairro/cidade e pede só o
    // número — evita rejeitar e pedir pra digitar o endereço inteiro de novo.
    if (step.field === 'endereco' && isCepOnly(trimmed)) {
      const cepBaseAddress = await lookupCep(trimmed);
      if (cepBaseAddress) {
        await sendText(waId, `Achei o endereço: ${cepBaseAddress}\n\nSó falta o número da casa/apartamento e complemento (se houver).`);
        await goTo(waId, 'AWAITING_RET_ENDERECO_NUMERO', { ...data, cepBaseAddress });
        return;
      }
      await sendText(waId, 'Não encontrei esse CEP. Pode mandar o endereço completo (rua, número, bairro e cidade)?');
      await askRetentionStep(waId, idx);
      return;
    }
    ...
    await advanceRetentionFlow(waId, contactId, state, { ...data, [step.field]: value });
    return;
  }

  // Resposta à pergunta "você está em casa?" mandada quando o motoboy avisa
  // que está a caminho (ver botão "A caminho" em lib/motoboy-conversation.js)
  // — avisa o motoboy de volta pelo telefone dele.
  if (state === 'AWAITING_HOME_CONFIRM') {
    if (input.kind !== 'interactive' || (input.id !== 'home_yes' && input.id !== 'home_no')) {
      await sendButtons(waId, 'Você se encontra em casa?', [
        { id: 'home_yes', title: 'Sim, estou' },
        { id: 'home_no', title: 'Não estou' },
      ]);
      return;
    }
    const isHome = input.id === 'home_yes';
    await sendText(waId, isHome ? 'Obrigado! Já avisei o motoboy. 🛵' : 'Sem problemas, já avisei o motoboy. 🛵');
    if (data.motoboyPhone) {
      if (isHome) {
        await sendText(data.motoboyPhone, '🏠 Cliente confirmou: está em casa.');
      } else {
        // Cliente não está — já resolve a parada como ausente sozinho e
        // mostra a próxima pro motoboy, em vez de só avisar e deixar ele
        // decidir (evita viagem perdida de verdade).
        try {
          await handleCustomerNotHome(data.caseId, data.motoboyPhone);
        } catch (err) {
          console.error('Erro ao pular parada automaticamente (cliente ausente):', err.message);
        }
      }
    }
    await clearConversationState(waId);
  ...
  if (state === 'AWAITING_DATE') {
    if (input.kind === 'interactive' && input.id === 'date_other') {
      await askOtherDate(waId);
      await goTo(waId, 'AWAITING_OTHER_DATE', data);
      return;
    }
  ...
  if (state === 'AWAITING_OTHER_DATE') {
    ...
    const periods = availablePeriodsForDate(iso);
    if (!periods.length) {
      await sendText(waId, 'Essa data não possui mais períodos disponíveis.');
      await askOtherDate(waId);
      return;
    }
    await askPeriod(waId, periods);
  ...
  if (state === 'AWAITING_PERIOD') {
    ...
    await syncPartialSchedule({ waId, pickupDate: data.pickupDate, period });
    await askResponsible(waId);
    await goTo(waId, 'AWAITING_RESPONSIBLE', { ...data, period });
    return;
  }
  ...
  if (state === 'AWAITING_ADDRESS_CONFIRM') {
    ...
    if (input.id === 'address_yes') {
      await askDate(waId);
      await goTo(waId, 'AWAITING_DATE', { ...data, address: data.knownAddress });
      return;
    }
  ...
  if (state === 'AWAITING_ADDRESS') {
    ...
    if (input.kind === 'text' && isCepOnly(input.value)) {
      const address = await lookupCep(input.value);
      if (!address) {
        await sendText(waId, 'Não encontrei esse CEP. Confira e envie novamente, ou informe o endereço completo.');
        return;
      }
      await sendText(waId, `Localizei: ${address}\n\nAgora informe o número e o complemento, se houver.`);
      await goTo(waId, 'AWAITING_ADDRESS_NUMBER', { ...data, address });
      return;
    }
    ...
    await askDate(waId);
    await goTo(waId, 'AWAITING_DATE', { ...data, address: input.value });
    return;
  }
  ...
  if (state === 'AWAITING_ADDRESS_NUMBER') {
    ...
    await askDate(waId);
    await goTo(waId, 'AWAITING_DATE', { ...data, address });
    return;
  }

  if (state === 'AWAITING_RESPONSIBLE') {
    if (input.kind !== 'interactive') {
      await askResponsible(waId);
      return;
    }
    if (input.id === 'responsible_self') {
      await askEquipmentPreparation(waId);
      await goTo(waId, 'AWAITING_EQUIPMENT', { ...data, responsibleName: 'Eu mesmo', contactPhone: waId });
      return;
    }
    if (input.id === 'responsible_other') {
      await sendText(waId, 'Informe o nome da pessoa responsável pela entrega.');
      await goTo(waId, 'AWAITING_RESPONSIBLE_NAME', data);
      return;
    }
    await askResponsible(waId);
    return;
  ...
  if (state === 'AWAITING_RESPONSIBLE_NAME') {
    if (input.kind !== 'text' || input.value.trim().length < 2) {
      await sendText(waId, 'Informe o nome da pessoa responsável pela entrega.');
      return;
    }
    await sendText(waId, 'Informe o telefone dessa pessoa com DDD. Se não quiser informar, responda “não”.');
    await goTo(waId, 'AWAITING_RESPONSIBLE_PHONE', {
      ...data,
      responsibleName: input.value.trim(),
    });
    return;
  }

  if (state === 'AWAITING_RESPONSIBLE_PHONE') {
    if (input.kind !== 'text') {
      await sendText(waId, 'Informe o telefone com DDD ou responda “não”.');
      return;
    }
    const skipped = normalize(input.value) === 'nao';
    const digits = input.value.replace(/\D/g, '');
    if (!skipped && digits.length < 10) {
      await sendText(waId, 'Esse telefone parece inválido. Envie com DDD ou responda “não”.');
      return;
    }
    await askEquipmentPreparation(waId);
    await goTo(waId, 'AWAITING_EQUIPMENT', {
      ...data,
      contactPhone: skipped ? waId : input.value.trim(),
    });
    return;
  }
  ...
  if (state === 'AWAITING_EQUIPMENT') {
    ...
    if (input.id === 'equipment_help') {
      await askEquipmentHelp(waId);
      await goTo(waId, 'AWAITING_EQUIPMENT_HELP', data);
      return;
    }
    if (input.id === 'equipment_ready') {
      const updated = { ...data, observacao: `Responsável: ${data.responsibleName || 'Eu mesmo'}. Equipamentos separados.` };
  ...
  if (state === 'AWAITING_EQUIPMENT_HELP') {
    if (input.kind !== 'interactive' || !input.id.startsWith('help_')) {
      await askEquipmentHelp(waId);
      return;
    }
    await sendText(waId, 'Entendi. Vou encaminhar sua situação para o atendimento humano continuar por aqui.');
    await goTo(waId, 'AWAITING_HUMAN_SUPPORT', { ...data, equipmentHelpReason: input.id });
    return;
  }
    ...
    if (input.id === 'confirm_yes') {
      ...
      await sendText(
        ...
        `Retirada agendada com sucesso ✅\n\nSua solicitação foi registrada.\n\nData: ${data.pickupDateLabel}\nPeríodo: ${
          PERIODS[`period_${data.period}`] || data.period
        }\nEndereço: ${data.address}\n\nDeixe os equipamentos separados e avise a pessoa responsável pela entrega.\n\nEnviaremos uma confirmação antes da chegada da equipe.`
      );
      await clearConversationState(waId);
      return;
    }
    if (input.id === 'confirm_no') {
      await sendText(waId, 'Sem problema. Vamos revisar desde o endereço.');
      await startPickupFlow(waId);
      return;
    }
    await askConfirmation(waId, data);
...
module.exports = { handleConversation, showPickupOpening, startPickupFlow, BRAND };

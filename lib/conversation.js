...
const { syncPickupScheduled, syncRetentionLead } = require('./new-schema-sync');
...
const RETENTION_STEPS = [
  ...
  { state: 'AWAITING_RET_PLANO', field: 'plano', prompt: `Qual plano você tem interesse (ex: velocidade ou o que já conhece da ${BRAND})?` },
...
async function handleConversation({ waId, contactId, msg, profileName }) {
  ...
  if (input.kind === 'button') {
    ...
    if (reason) {
      ...
      await setConversationState(waId, 'AWAITING_RETENTION_OFFER', { ...data, motivo: reason.id });
      return;
    }
  }
  ...
  if (state === 'AWAITING_RETENTION_REASON') {
    ...
    await setConversationState(waId, 'AWAITING_RETENTION_OFFER', { ...data, motivo: input.id });
    return;
  }
  ...
  if (state === 'AWAITING_RETENTION_OFFER') {
    ...
    if (input.id === 'retention_yes') {
      ...
      await setConversationState(waId, RETENTION_STEPS[0].state, { motivo: data.motivo });
      return;
    }
    if (input.id === 'retention_no') {
      await askRetentionLastChance(waId);
      await setConversationState(waId, 'AWAITING_RETENTION_LASTCHANCE', data);
      return;
    }
    await askRetentionPitch(waId, data.motivo);
    return;
  ...
  if (state && state.startsWith('AWAITING_RET_')) {
    ...
    const value = step.field === 'telefone2' && /^n[aã]o$/i.test(input.value.trim()) ? null : input.value.trim();
    const updated = { ...data, [step.field]: value };

    const nextIdx = idx + 1;
    if (nextIdx < RETENTION_STEPS.length) {
      await askRetentionStep(waId, nextIdx);
      await setConversationState(waId, RETENTION_STEPS[nextIdx].state, updated);
      return;
    }
  ...
  if (state === 'AWAITING_PHONE') {
    ...
    await setConversationState(waId, 'AWAITING_OBSERVATION', updated);
    return;
  }

...
const EXPANDED_REGION_CONTROL_WA_ID = '5519983415710';
const EXPANDED_REGION_CITIES = new Set([
  'americana',
...
function appointmentRecipient(city) {
  return EXPANDED_REGION_CITIES.has(normalizeCity(city))
    ? EXPANDED_REGION_CONTROL_WA_ID
    : CONTROL_WA_ID;
}
...
async function claimAndSend(pool, row) {
  const recipientWaId = appointmentRecipient(row.city);
  const { rowCount } = await pool.query(
    `insert into whatsapp_dispatch_log (appointment_id, recipient_wa_id)
     values ($1, $2)
     on conflict (appointment_id) do nothing`,
    [row.id, recipientWaId]
  );
  if (!rowCount) return false;

  try {
    const response = await sendText(recipientWaId, formatAppointment(row));

...
async function appendStopToRoute(pool, routeId, caseId) {
  ...
  // Se a rota já tinha sido dada como concluída antes (ex: só tinha 1 parada
  // e foi resolvida) e agora ganhou uma parada nova, reabre — senão o
  // motoboy nunca mais vê essa parada (getCurrentStop só busca rotas
  // PLANEJADA/EM_ANDAMENTO).
  await pool.query(`update routes set status = 'PLANEJADA', updated_at = now() where id = $1 and status = 'CONCLUIDA'`, [
    routeId,
  ]);
...
async function reoptimizeRoute(pool, routeId) {
  // Um cliente pode ter mais de um endereço geocodificado (mais de um caso
  // ativo pra ele, ex: vários agendamentos no mesmo telefone) — pega o
  // endereço que bate com o texto do agendamento desse caso específico,
  // e só cai pro mais recente do cliente se não achar (evita multiplicar
  // parada por causa de múltiplos endereços do mesmo cliente).
  const { rows: stops } = await pool.query(
    `select rs.id, rs.status,
            coalesce(matching.latitude, fallback.latitude) as latitude,
            coalesce(matching.longitude, fallback.longitude) as longitude
     from route_stops rs
     join case_records cr on cr.id = rs.case_id
     join service_orders so on so.id = cr.service_order_id
     join customers c on c.id = so.customer_id
     left join appointments ap on ap.case_id = cr.id
     left join lateral (
       select latitude, longitude from customer_addresses
       where customer_id = c.id and latitude is not null and full_address = ap.address
       limit 1
     ) matching on true
     left join lateral (
       select latitude, longitude from customer_addresses
       where customer_id = c.id and latitude is not null
